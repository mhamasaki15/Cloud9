Feature: Home Page
	...
	Scenario: R1When the text box is empty search not enabled
		Given I am on the home page
		When "" is in the input box
		Then the search button should not be enabled

	Scenario: R2Input box is editable
		Given I am on the home page
		When I click on the input box
		And enter a character
		Then that character should populate the input box

	Scenario: R3Everytime user edits text in box the search should be set to no selection
